...
class NotificationService {
  ...
  static Future<void> initialize() async {
    ...
    await ap?.createNotificationChannel(const AndroidNotificationChannel(
      ...
      importance: Importance.low, playSound: false,
    ));
    await ap?.createNotificationChannel(const AndroidNotificationChannel(
  ...
  static Future<void> scheduleNotification({
    ...
    const details = NotificationDetails(android: AndroidNotificationDetails(
      ...
      fullScreenIntent: true, playSound: true, enableVibration: true,
    ));
    try {
      await _plugin.zonedSchedule(id, title, body, tzDate, details,
  ...
  static Future<void> showTimerNotification(String chapterName) async {
    await initialize();
    await _plugin.show(88888, 'Studying Now', 'Session: $chapterName',
      const NotificationDetails(android: AndroidNotificationDetails(
        'study_timer', 'Study Timer',
        importance: Importance.low, priority: Priority.low,
        ongoing: true, autoCancel: false, playSound: false,
      )));
  }

  static Future<void> cancelTimerNotification() async => _plugin.cancel(88888);
